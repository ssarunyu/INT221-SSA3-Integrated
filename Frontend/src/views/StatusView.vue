...
const fetch = async() => {
    // Fetch data
    const result = await getData(import.meta.env.VITE_STATUS_URL)
    // Front-end
    taskManagement.value.addAllStatus(result)
}
...
const editPopupStatus = async (statusId) => {
    router.push({name: 'EditStatusPopup',params: { editStatus : statusId}})
...
<template>
    <AddStatusPopup v-show="addStatusPopup" @close="closeAddStatusPopup()" @confirm="confirmAddStatus()"/>
    ...
    <div class="w-full min-h-screen p-5">
        ...
        <div v-for="item in taskManagement.getAllStatus()" class="itbkk-item relative p-3 rounded">
            ...
            <div class="itbkk-status-action flex col-span-1 " v-if="item.name !== 'No Status'">
                <div @click="editPopupStatus(item.id)" class="itbkk-button-edit p-2 px-5 w-35 m-1 rounded cursor-pointer duration-300 bg-gray-300 hover:bg-gray-400 hover:scale-105">
                    Edit
                </div>
                <div @click="" class="itbkk-button-delete p-2 px-3 w-35 m-1 rounded cursor-pointer duration-300 bg-gray-300 hover:bg-gray-400 hover:scale-105">
                    Delete
                </div>
            </div>
        </div>
...
<!-- //  <template>
//     <AddStatusPopup v-show="addStatusPopup" @close="closeAddStatusPopup()" @confirm="confirmAddStatus()"/>
//     <EditStatusPopup v-show="editStatusPopup" v-if="targetItem" :itemData="targetItem"  @close="closeEditPopupStatus()" />
//     <div class="w-full min-h-screen p-5">
//         <h1 class="flex text-2xl font-bold justify-center mb-5">ITBKK SSA3 Taskboard</h1>
//         <div class="flex w-full justify-between">
//             <div class="flex p-3">
